...
MCP 도구 `review_episode` / `batch_review`를 통해 외부 AI(Gemini/GPT/NIM/Ollama) 편집 리뷰를 오케스트레이션한다. Gemini + GPT가 1차 기준, NIM/Ollama는 객관적 오류 보충용. CLAUDE.md와 충돌하는 제안은 건너뛴다. 본문을 직접 수정하지 않고 수정안만 제시한다.

> **필수 참조**: `CLAUDE.md`(플래그: nim_feedback, ollama_feedback, gpt_feedback, illustration), `settings/`(01, 03, 04), `summaries/editor-feedback-log.md`
> **편집 지시문**: `GEMINI.md`와 `GPT.md`는 `novel-editor` MCP 서버에 번들되어 있으며, MCP 도구 호출 시 자동으로 적용된다.
> **실행 순서**: MCP 도구가 자동 관리 (Phase 1: NIM/Ollama 병렬 → Phase 2: Gemini + GPT 병렬). 실패 시 건너뛰고, 다음 묶음 검토(P7)에서 재시도한다.
> **소스별 신뢰도**: Gemini(높음, 연속성/세계관) / GPT(높음, 문체/대화/감정선) / NIM(중간) / Ollama(낮음, 보조).
...
gpt_feedback: true/false  (기본: true)
```
...
> **도구가 자동으로 수행하는 것:**
> - CLAUDE.md에서 nim_feedback/ollama_feedback/gpt_feedback 플래그 파싱
> - Phase 1: NIM + Ollama 병렬 호출 (활성화된 소스만)
> - Phase 2: Gemini + GPT(Codex) 병렬 호출 (Phase 1 결과를 참고 자료로 전달)
> - 실패 소스는 건너뛰고 로그에 기록. 다음 묶음 검토(P7)에서 재시도
> - `EDITOR_FEEDBACK_gemini.md`, `EDITOR_FEEDBACK_gpt.md`, `EDITOR_FEEDBACK_nim.md` 등 자동 저장
> - 결과 요약 반환
...
**Gemini + GPT 피드백**을 기준으로 각 항목을 평가한다. Gemini는 연속성/세계관, GPT는 문체/대화/감정선이 주력이다. NIM/Ollama 피드백에서 Gemini/GPT가 놓친 객관적 오류가 있으면 추가로 반영한다.
...
3. **피드백 충돌 시 우선순위**: CLAUDE.md > settings/ > 이전 에피소드 본문 > Gemini 피드백 = GPT 피드백 (영역별) > NIM 피드백 > Ollama 피드백.
