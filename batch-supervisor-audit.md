...
이 파일이 소설 폴더 안에 위치하므로, 감독자는 아래 변수를 **파일 경로에서 자동으로 추론**한다. 수동 설정 불필요.

| 변수 | 추론 규칙 | 예시 |
|------|-----------|------|
| `NOVEL_ID` | 이 파일이 위치한 폴더명 | `no-title-013` |
| `SESSION` | `audit-{NOVEL_ID의 숫자}` | `audit-013` |
| `NOVEL_DIR` | 이 파일의 절대 경로에서 파일명 제외 | `/root/novel/no-title-013` |

사용자가 프롬프트에서 지정하는 값:

| 변수 | 설명 | 예시 |
|------|------|------|
| `BATCH_SIZE` (= N) | 배치당 감사 화수 | `10` |
...
이 파일의 경로에서 NOVEL_ID, SESSION, NOVEL_DIR을 추론한 뒤, 배치 감사를 감독한다. 아래 규칙대로 진행해.

### 0. 변수 추론

이 파일의 경로에서 변수를 결정한다:
- **NOVEL_ID**: 이 파일이 위치한 폴더명 (예: `no-title-013`)
- **SESSION**: `audit-{숫자 부분}` (예: `no-title-013` → `audit-013`)
- **NOVEL_DIR**: 이 파일의 절대 경로에서 파일명을 제거 (예: `/root/novel/no-title-013`)

**다른 소설의 세션(audit-001 등)이 이미 존재해도 무시한다. 반드시 위에서 추론한 SESSION만 사용한다.**

### 1. 세션 관리

- tmux 세션명: 위에서 추론한 `SESSION`
