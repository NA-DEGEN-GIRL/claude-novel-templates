...
- **외부 AI 편집**: Gemini CLI 연동으로 이중 검토 (Claude가 쓰고, Gemini가 편집)
...
| [Gemini CLI](https://github.com/google-gemini/gemini-cli) | 외부 AI 편집자 (선택) | `npm install -g @anthropic-ai/gemini` |
...
| **gemini-feedback** | Gemini CLI로 외부 편집 리뷰 → 반영/참고/건너뜀 판단 | writer가 자동 호출 |
...
Claude가 쓴 원고를 Gemini가 편집 리뷰한다. [GEMINI.md](./GEMINI.md)가 Gemini의 편집 지시문이다.

### 동작 방식

1. writer가 집필 완료 후 `gemini-feedback` 에이전트를 호출
2. Gemini CLI가 에피소드를 분석하고 `EDITOR_FEEDBACK.md`에 피드백 작성
3. Claude가 피드백을 평가하여 반영/참고/건너뜀 결정
4. `CLAUDE.md`(소설 규칙)와 충돌하는 제안은 자동으로 건너뜀
...
Gemini CLI가 설치되어 있어야 한다. 없으면 이 단계는 자동으로 건너뛴다.
